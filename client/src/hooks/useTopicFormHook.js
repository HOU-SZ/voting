...
export const useTopicFormHook = (props) => {
    ...
    useEffect(() => {
        if (!topicId) resetFormControls();
            ...
            createAPIEndpoint(ENDPOINTS.TOPICS).fetchById(topicId)
                .then(res => {
    ...
    const validateForm = () => {
        let temp = {};
        temp.TopicContent = values.TopicContent === "" ? "This filed is required" : values.TopicContent.length<=200 ? "" : "Max 200 Characters can accept.";
        temp.IsAnonymous = values.IsAnonymous !== "" ? "" : "This field is required.";
        temp.OneLikePerIdea = values.OneLikePerIdea !== "" ? "" : "This field is required.";
        temp.NumberColumn = parseInt(values.NumberColumn) !== 0 ? "" : "This field is required.";
        temp.MaxLikePerUser = parseInt(values.MaxLikePerUser) !== 0 ? "" : "This field is required.";
        setErrors({ ...temp });
        console.log(values);
    ...
    const submitTopic = e => {
        ...
        if (validateForm()){
            ...
            if (!topicId){
                ...
                createAPIEndpoint(ENDPOINTS.TOPICS).create(updatedValues)
                    .then(res => {
                        resetFormControls();
                ...
                createAPIEndpoint(ENDPOINTS.TOPICS).update(updatedValues)
                    .then(res => {
                        resetFormControls();
                        setSubmitted(submitted+1);
                        setTopicId(0);
    ...
    return {
        HandleChange, resetFormControls, submitTopic, setTopicId, setSubmitted, topicId, submitted, values, errors
    }
}