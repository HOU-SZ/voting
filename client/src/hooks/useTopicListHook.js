...
export const useTopicListHook = (props) => {
    ...
    useEffect(() => {
        ...
        setCurrentPage(1);
        // createAPIEndpoint(ENDPOINTS.TOPICS).fetchAll()
        //     .then(res => {
        //         let topicList = res.data.map(item => ({
        //             TopicContent: item.topicContent,
        //             UserId: item.userId,
        //             TopicId: encodeURIComponent(CryptoJS.AES.encrypt(item.topicId.toString(), 'VoTeam_2021').toString()),
        ...
        //         setTotalPage(Math.floor((topicList.length-1)/10)+1);
        //         setCurrentList(topicList.slice(0,10));
        //         setCurrentPage(1);
    ...
    const editTopic = topicId => {
        var bytes  = CryptoJS.AES.decrypt(decodeURIComponent(topicId), 'VoTeam_2021');
        var trueTopicId = parseInt(bytes.toString(CryptoJS.enc.Utf8));
    ...
    const deleteTopic = (topicId) => {
        var bytes  = CryptoJS.AES.decrypt(decodeURIComponent(topicId), 'VoTeam_2021');
        var trueTopicId = parseInt(bytes.toString(CryptoJS.enc.Utf8));
        createAPIEndpoint(ENDPOINTS.TOPICS).delete(trueTopicId)
            .then(res => {
    ...
    const setNext = (currentPage) => {
        if (currentPage < totalPage ) {
            setCurrentList(topicList.slice(currentPage*10,(currentPage+1)*10));
    ...
    const setPrev = (currentPage) => {
        if (currentPage > 1 ) {
            setCurrentList(topicList.slice((currentPage-2)*10, (currentPage-1)*10));
            setCurrentPage(currentPage-1);
        }
    }
    
    return {
        editTopic, deleteTopic, setNext, setPrev, currentList, currentPage, totalPage
    }
}